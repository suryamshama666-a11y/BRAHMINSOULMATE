...
-- Date: 2026-04-13
-- Purpose: Resolve database schema inconsistencies and add missing indexes
...
CREATE POLICY "Users can view own connections" ON connections
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id_1 FROM connections WHERE id = connections.id
            UNION
            SELECT user_id_2 FROM connections WHERE id = connections.id
        )
    );
