...
async function createProfilesWithoutAuth() {
  console.log('🔄 Alternative: Creating profiles without auth users...\n');
  
  // Try creating profiles with NULL user_id or without foreign key constraint
  ...
  for (let i = 0; i < simpleProfiles.length; i++) {
    const profile = simpleProfiles[i];
    console.log(`📋 Creating simple profile ${i + 1}`);
...
async function main() {
  console.log('🚀 COMPLETE Database Population (Users + Profiles)');
  ...
  try {
    ...
    // Try Method 1: Create auth users + profiles
    console.log('🎯 Method 1: Creating auth users + profiles...');
    const { successCount, errorCount } = await createUsersAndProfiles();
    
    if (successCount === 0) {
      console.log('\n🎯 Method 2: Creating profiles without auth users...');
      const simpleSuccess = await createProfilesWithoutAuth();
      console.log(`Simple profiles created: ${simpleSuccess}`);
    }
